...
export class FactoryProvider extends BaseServiceProvider {
	inject?: any[];
	useFactory: Function;
...
export class ClassProvider extends BaseServiceProvider {
	useClass: any;
	...
	public createInstance(moduleName: string, provide: string) {
		const instance = this.moduleMetadata.getServiceInstance(
			moduleName,
			provide
		);
		if (instance) {
